...
UdpServiceDiscovery::~UdpServiceDiscovery() {
    Stop();
}
...
void UdpServiceDiscovery::Stop() {
    ...
    if (receiving_) {
        ...
        if (recv_socket_) {
            asio::error_code ec; recv_socket_->close(ec);
        }
    ...
    if (broadcasting_) {
        ...
        if (bcast_socket_) {
            asio::error_code ec; bcast_socket_->close(ec);
        }
...
std::vector<EndpointIdentity> UdpServiceDiscovery::DiscoverServices(const std::string& service_name, ServiceDiscoveredCallback callback) {
    if (callback) {
...
void UdpServiceDiscovery::ProcessDiscoveryMessage(const std::vector<uint8_t>& data, 
    ...
    try {
        ...
        if (msg_type == "service_discovery") {
            ...
            LOG_DEBUG_STREAM << "[SD] 收到服务发现消息 -> id=" << service_info.id 
                             << ", address=" << service_info.address << ":" << service_info.port;
...
void UdpServiceDiscovery::EnsureReceiveStarted() {
    ...
    try {
        if (!recv_socket_) {
            ...
            recv_socket_->bind(asio::ip::udp::endpoint(
                asio::ip::make_address(config_.local_address),
        ...
        if (!recv_thread_.joinable()) {
            recv_thread_ = std::thread([ctx = recv_io_]() {
                try { ctx->run(); } catch (const std::exception& e) { std::cerr << "UDP接收线程异常: " << e.what() << std::endl; }
            });
        }
    ...
    try {
        ...
        if (!bcast_thread_.joinable()) {
            bcast_thread_ = std::thread([ctx = bcast_io_]() {
                try { ctx->run(); } catch (const std::exception& e) { std::cerr << "UDP广播线程异常: " << e.what() << std::endl; }
            });
        }
