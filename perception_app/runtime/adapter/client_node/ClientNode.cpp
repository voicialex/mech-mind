...
ClientNode::ClientNode(const Config &config) : config_(config) {}
...
bool ClientNode::Initialize() {
  ...
  try {
    ...
    // 设备客户端配置
    EndpointIdentity device_config;
    ...
    if (config_.enable_controller_client) {
      EndpointIdentity controller_config;
    ...
    std::cout << "ClientNode RegisterEventHandler" << std::endl;
...
// 注册消息回调函数
void ClientNode::RegisterMessageCallback(MessageType message_type, uint16_t message_id, uint8_t sub_message_id,
                                         MessageCallback callback) {
  if (message_router_) {
    message_router_->RegisterCallback(message_type, message_id, sub_message_id, std::move(callback));
  }
}
...
void ClientNode::OnServiceDiscovered(const EndpointIdentity &service_info) {
  LOG_INFO_STREAM << "发现服务 - ID: " << service_info.id << ", 名称: " << service_info.name
  ...
  if (service_info.type == EndpointType::Server) {
    // 连接到设备服务器
    if (service_info.id.find("device_server") != std::string::npos ||
        service_info.name.find("Device Server") != std::string::npos ||
        service_info.name.find("Center Device Server") != std::string::npos) {
      LOG_INFO_STREAM << "匹配到设备服务，尝试连接: " << service_info.address << ":" << service_info.port;
      if (device_client_) {
        device_client_->ConnectToServer(service_info.address, service_info.port);
      }
    }
    // 控制器服务器
    if (controller_client_ && (service_info.id.find("controller_server") != std::string::npos ||
                               service_info.name.find("Controller Server") != std::string::npos)) {
      LOG_INFO_STREAM << "匹配到控制器服务，尝试连接: " << service_info.address << ":" << service_info.port;
...
// 设备客户端事件处理器实现
void ClientNode::DeviceClientEventHandler::OnMessageReceived(const std::string &endpoint_id,
                                                             const std::vector<uint8_t> &message_data) {
  if (!node_) return;
  LOG_INFO_STREAM << "[Device] 收到消息 - 端点: " << endpoint_id << ", 数据大小: " << message_data.size() << " 字节";
  ...
  if (node_->message_router_) {
    try {
      ...
      LOG_ERROR_STREAM << "[Device] 消息处理异常: " << e.what();
    }
  }
}
void ClientNode::DeviceClientEventHandler::OnConnectionChanged(const std::string &endpoint_id, bool connected,
...
void ClientNode::DeviceClientEventHandler::OnError(const std::string &endpoint_id, uint16_t error_code,
                                                   const std::string &error_message) {
  if (!node_) return;
  LOG_ERROR_STREAM << "[Device] 错误 - 端点: " << endpoint_id << ", 错误码: " << error_code
                   << ", 错误信息: " << error_message;
}

// 控制器客户端事件处理器实现
void ClientNode::ControllerClientEventHandler::OnMessageReceived(const std::string &endpoint_id,
                                                                 const std::vector<uint8_t> &message_data) {
  if (!node_) return;
  LOG_INFO_STREAM << "[Controller] 收到消息 - 端点: " << endpoint_id << ", 数据大小: " << message_data.size()
                  << " 字节";
  ...
  if (node_->message_router_) {
    try {
      ...
      LOG_ERROR_STREAM << "[Controller] 消息处理异常: " << e.what();
    }
  }
}
void ClientNode::ControllerClientEventHandler::OnConnectionChanged(const std::string &endpoint_id, bool connected,
                                                                   const ConnectionInfo &connection_info) {
  if (!node_) return;
  if (connected) {
...
void ClientNode::ControllerClientEventHandler::OnError(const std::string &endpoint_id, uint16_t error_code,
                                                       const std::string &error_message) {
  if (!node_) return;
  LOG_ERROR_STREAM << "[Controller] 错误 - 端点: " << endpoint_id << ", 错误码: " << error_code
                   << ", 错误信息: " << error_message;
}
